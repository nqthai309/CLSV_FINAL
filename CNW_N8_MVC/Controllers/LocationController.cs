...
namespace CNW_N8_MVC.Controllers
{
    public class LocationController : Controller
    {
        ...
        public ActionResult HotelList(string locationName, int? page)
        {
            ...
            var model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndGetListHotelByLocation(locationName)).ToPagedList(page ?? 1, 9);
            ViewData["count"] = model.Count.ToString();

            var optionList = JsonConvert.DeserializeObject<List<locations>>(client.FrontEndGetListLocation());
        ...
        public ActionResult HomestayList(string locationName, int? page)
        {
            setUsername();
            ViewData["locationName"] = locationName;
            var model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndGetListHomestayByLocation(locationName)).ToPagedList(page ?? 1, 9);
            ViewData["count"] = model.Count.ToString();
