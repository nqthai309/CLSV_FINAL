...
namespace CNW_N8_MVC.Controllers
{
    public class HotelController : Controller
    {
        ...
        public ActionResult List(int? page)
        {
            ...
            setUsername();
            var optionList = JsonConvert.DeserializeObject<List<locations>>(client.FrontEndOpitionList());
            //var optionList = context.locations.Where(o => o.id != null).ToList();
            ViewData["optionList"] = optionList;

            var model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndGetListHotels(page)).ToPagedList(page ?? 1, 9);
        ...
        public ActionResult SearchEngine(int? page)
        {
            ...
            if (priceRange == "Dưới 300.000VNĐ")
            {
                ...
                model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndSearchEngine_Price1(page, txtSearch, locationSelect)).ToPagedList(page ?? 1, 9);

            }
            if (priceRange == "300.000- 500.000VNĐ")
            {

                model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndSearchEngine_Price2(page, txtSearch, locationSelect)).ToPagedList(page ?? 1, 9);
            ...
            if (priceRange == "500.000- 1.000.000VNĐ")
            {
                ...
                model = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndSearchEngine_Price3(page, txtSearch, locationSelect)).ToPagedList(page ?? 1, 9);
        ...
        public ActionResult Detail(int? id)
        {
            ...
            if (id.HasValue)
            {
                ...
                model.hotel = JsonConvert.DeserializeObject<hotels>(client.FrontEndGetDetialHotel(id));
                //model.hotels = context.hotels.Where(x => x.id != 0).ToList();
                model.hotels = JsonConvert.DeserializeObject<List<hotels>>(client.FrontEndGetDetialHotels());
                ...
                model.dichVu = JsonConvert.DeserializeObject<List<hotel_service>>(client.FrontEndGetDichVuHotel(id));
        ...
        public ActionResult AddItemHotel(int id)
        {
            ...
            else
            {
                ...
                var hotel = JsonConvert.DeserializeObject<hotels>(client.FrontEndAddItemHotel(id));
                var dichVu = JsonConvert.DeserializeObject<List<hotel_service>>(client.FrontEndGetDichVuHotel(id));
                foreach (var it in dichVu)
                {
                    if(it.service_id == 1)
                    {
                        selectDichVu += it.services.service_name + ',';
                        thanhTienDichVu += (int)it.unit_price;
                    }
                }
